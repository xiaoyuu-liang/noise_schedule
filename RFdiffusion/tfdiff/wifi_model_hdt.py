...
class MLPConditionEmbedding(nn.Module):
    def __init__(self, cond_dim, hidden_dim=256):
        super().__init__()
        # print(f"IN MLPConditionEmbedding: cond_dim: {cond_dim}, hidden_dim: {hidden_dim}")
        self.projection = nn.Sequential(
...
class DiA(nn.Module):
    ...
    def forward(self, x, t, c):
        ...
        # print(f"IN DiA after s_attn: x: {x.shape}, t: {t.shape}, c: {c.shape}")
        x = x + self.x_attn(queries=self.normc(c),
                            keys=self.norm2(x), values=self.norm2(x))
        # print(f"IN DiA after x_attn: x: {x.shape}, t: {t.shape}, c: {c.shape}")
        x = x + \
            gate_mlp.unsqueeze(
...
class SpatialDiffusion(nn.Module):
    ...
    def __init__(self, params):
        ...
        # self.cond_dim = params.cond_dim  # C
    ...
    def forward(self, x, t, c):
        ...
        t = self.t_embed(t)
        c = self.c_embed(c)
        # print(f"IN SP: x: {x.shape}, t:{t.shape}, c: {c.shape}")
...
class TimeFrequencyDiffusion(nn.Module):
    ...
    def forward(self, x, t, c):
        x = self.p_embed(x)
        t = self.t_embed(t)
        # print(f"IN TF : x: {x.shape}, t:{t.shape}, c: {c.shape}")
        if self.task_id == 0:
            c = c.reshape([x.shape[0], self.input_len, -1, 2])
        if self.task_id == 1:
...
class tfdiff_WiFi(nn.Module):
    ...
    def forward(self, x, t, c):
        # print(f"IN HDT: x: {x.shape}, t:{t.shape}, c: {c.shape}")
        x_s = x.reshape([-1]+self.extra_dim[::1]+[2]).transpose(0, 2, 1, 3)  # [B*N, S, A, 2]
        c_s = c.repeat(self.sample_rate*self.cond_dim[-2], 1, 1).reshape([-1]+self.cond_dim+[2])  # [B*N, [C], 2] = [B*N, S, C, 2]
        # print(f"IN HDT: x_s: {x_s.shape}, t_s:{t.repeat(self.sample_rate).shape}, c_s: {c_s.shape}")
        ...
        # print(f"IN HDT after SP: x: {x.shape}")
        # x = self.tf_block(x, t, c)  # [B, N, S*A, 2]
        x = self.tf_block(x, t, c_s.reshape([-1]+[np.prod(self.cond_dim)]+[2]))  # [B, N, S*A, 2]
        x = x.reshape([-1, self.sample_rate]+[self.extra_dim[-1]*self.extra_dim[-2]]+[2])  # [B, N, S, A, 2]
        # print(f"IN HDT after TF: x: {x.shape}")
        return x
