...
class tfdiffLoss(nn.Module):
    ...
    def forward(self, target, est, target_noise=None, est_noise=None):
        ...
        est_fft = torch.fft.fft(est, dim=1)
        ...
        loss = t_loss + f_loss + self.w * n_loss
        return {'loss': loss, 't_loss': t_loss, 'f_loss': f_loss}
...
class tfdiffLearner:
    ...
    def save_to_checkpoint(self, filename='weights'):
        save_basename = f'{filename}-{self.iter//len(self.dataset)}.pt'
