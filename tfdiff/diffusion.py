...
class SignalDiffusion(nn.Module):
    ...
    def robust_sampling(self, restore_fn, cond, device):
        batch_size = cond.shape[0] # B
        sample_rate = self.params.sample_rate # N
        batch_max = (self.max_step-1)*torch.ones(batch_size, dtype=torch.int64)
        # Add batch dimension.
        # cond = torch.view_as_real(torch.from_numpy(cond['cond']).to(torch.complex64)).unsqueeze(0)
        # Construct a mini-batch.
        ...
        for s in range(self.max_step-1, -1, -1): # reverse from t to 0
            ...
            if s > 0:
                ...
                # x_s = x_s.reshape(batch_size, sample_rate, -1, 2) - self.degrade_fn(x_0_hat, t=s*torch.ones(batch_size, dtype=torch.int64),task_id = self.task_id) + self.degrade_fn(x_0_hat, t=(s-1)*torch.ones(batch_size, dtype=torch.int64),task_id = self.task_id) # degrade \hat{x_0} to x_{s-1}
